...
发布状态：已通过 GitHub Actions 自动发布 npm prerelease 至 npmjs.com（2026-08-21 起，当前 `0.1.0-rc.3`），并初始化 GitHub 远程 `github.com/Lan-zk/dsh-architecture-mentor`（提交已推送）。安装命令见根 README 与 `dsh-engineering-design.md` §4。
...
- M5 试点延后；npm prerelease 已通过 CI 发布（当前 `0.1.0-rc.3`），正式版与 GitHub Release 仍延后，触发条件保留在 docs/architecture-mentor/README.md 与实施计划相关记录中。
