...
发布状态：已通过 GitHub Actions 发布 npm 正式版 `dsh-architecture-mentor@0.1.2` 至 npmjs.com（2026-08-21；此前 rc.1–rc.3 为 CI 测试版，0.1.1 修复了 preset/VERSION 内部版本未同步的问题，0.1.2 修复了 npm 全局 bin symlink 下安装器静默退出的问题），并初始化 GitHub 远程 `github.com/Lan-zk/dsh-architecture-mentor`（提交已推送）。安装命令见根 README 与 `dsh-engineering-design.md` §4。
...
- M5 试点延后；npm 正式版 `0.1.2` 已通过 CI 发布；GitHub Release 仍延后（可选），触发条件保留在 docs/architecture-mentor/README.md 与实施计划相关记录中。
