...
| D6 | Active Context 预算 | 试点参数：`active-context.md` ≤ 8 KB；排序 = 决策价值 × 证据质量 × 误用风险；试点时用真实数据校准后回写文档（试点当前延后） | 记忆生命周期开放项 |
...
| D10 | 发布渠道 | 延后决定。试点通过后再定（GitHub Release 优先候选，npm registry 后置） | 工程化设计 §9 |
...
- 6 个技能在 M3 由执行者按编写规范 §11 自审并留档；原计划的"门禁相关与写权限内容人工复核"由用户决定跳过（2026-08-21），以 §11 机械自审记录为当前验收证据，试点时用真实行为再验证（借用 dsh `human-review-skill-maintenance` 的原则，不搬其实现）；
...
| 门禁"导师自检"自我放行 | 证据质量下降 | 必填字段 + 用户复核 + git 审计；试点时验证拒绝路径 |
